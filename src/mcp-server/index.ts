...
export function buildResearchQueries(topic: string, requiredCoverage: string[]): string[] {
    const coverageQueries = requiredCoverage.map((area) => `${topic} ${area}`);
    return [...new Set([topic, ...coverageQueries])].slice(0, 8);
}
