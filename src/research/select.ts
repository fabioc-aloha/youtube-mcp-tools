...
const GENERIC_COOKING_TERMS = new Set('bake baking cook cooking recipe recipes food dish easy best great good homemade home'.split(' '));

export function selectResearchCollection(brief: ResearchBrief, candidates: VideoCandidate[]): ResearchCollection {
    validateBrief(brief);
    const evaluated = candidates.map((candidate) => evaluateCandidate(brief, candidate));
    const eligible = evaluated.filter((candidate) => candidate.evidence.every((item) => item.passed));
    ...
    return {
        ...
        selectionMethod: 'Evidence-gated relevance, required-coverage matching, transcript availability, duration, source identity, optional recency, source diversity, redundancy reduction, and coverage optimization. Generic stop words and broad cooking terms are excluded from relevance and coverage evidence.',
        ...
        comparisonOpportunities: findComparisonOpportunities(selected),
    };
}

export function evaluateCandidate(brief: ResearchBrief, video: VideoCandidate): EvaluatedVideo {
    const topicTerms = meaningfulTerms(brief.topic);
    const normalizedText = normalize(`${video.title} ${video.tags?.join(' ') ?? ''} ${video.transcript ?? ''}`);
    const matchedTopicTerms = topicTerms.filter((term) => normalizedText.includes(term));
    const coveredAreas = brief.requiredCoverage.filter((area) => coverageMatch(area, normalizedText));
    ...
    return { video, relevanceScore, qualityScore, selectionScore: relevanceScore * TOPIC_WEIGHT + coverageScore * COVERAGE_WEIGHT + qualityScore * QUALITY_WEIGHT, matchedTopicTerms, coveredAreas, evidence };
}

function coverageMatch(area: string, normalizedText: string): boolean {
    const areaTerms = meaningfulTerms(area);
    if (areaTerms.length === 0) return false;
    const matched = areaTerms.filter((term) => normalizedText.includes(term));
    const distinctive = areaTerms.filter((term) => term.length >= 6 && !GENERIC_COOKING_TERMS.has(term));
    if (distinctive.some((term) => normalizedText.includes(term))) return true;
    return matched.length >= Math.min(2, areaTerms.length);
}
...
function similarity(left: VideoCandidate, right: VideoCandidate): number {
    const leftTerms = new Set(meaningfulTerms(`${left.title} ${left.tags?.join(' ') ?? ''}`));
    const rightTerms = new Set(meaningfulTerms(`${right.title} ${right.tags?.join(' ') ?? ''}`));
...
function findComplementarySources(selected: EvaluatedVideo[]): Array<{ videoId: string; reason: string }> {
    ...
    return selected.slice(1).filter((candidate) => candidate.coveredAreas.some((area) => !primary.coveredAreas.includes(area)) || candidate.video.channel !== primary.video.channel).map((candidate) => ({
        videoId: candidate.video.id,
        reason: candidate.video.channel !== primary.video.channel ? `Adds an independent source perspective from ${candidate.video.channel}.` : 'Adds coverage not fully represented by the primary selection.',
    }));
}
...
function findComparisonOpportunities(selected: EvaluatedVideo[]): Array<{ topic: string; videoIds: string[]; description: string }> {
    ...
    for (let index = 0; index < selected.length; index += 1) {
        for (let other = index + 1; other < selected.length; other += 1) {
            const shared = selected[index].coveredAreas.filter((area) => selected[other].coveredAreas.includes(area));
            if (shared.length > 0) opportunities.push({ topic: shared[0], videoIds: [selected[index].video.id, selected[other].video.id], description: 'Both sources address this coverage area; compare their explanations and evidence rather than assuming they are interchangeable.' });
        }
    }
...
function buildEvidence(brief: ResearchBrief, video: VideoCandidate, matchedTopicTerms: string[], coveredAreas: string[]): EvidenceItem[] {
    const evidence: EvidenceItem[] = [
        ...
        { criterion: 'Topic relevance', passed: matchedTopicTerms.length > 0, detail: matchedTopicTerms.length > 0 ? `Matched meaningful topic terms: ${matchedTopicTerms.join(', ')}.` : 'No meaningful topic terms matched the available title, tags, or transcript.' },
        { criterion: 'Required coverage', passed: coveredAreas.length > 0, detail: coveredAreas.length > 0 ? `Contributes evidence to: ${coveredAreas.join(', ')}.` : 'Does not contribute evidence to a requested coverage area.' },
...
function meaningfulTerms(text: string): string[] { return [...new Set(normalize(text).split(' ').filter((term) => term.length > 2 && !STOP_WORDS.has(term)))]; }
